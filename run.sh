...
ITER=10
GEN_LEN="128" #,32,128"
INPUT_LEN="3072" #"512,1024,2048,3072"
...
do
    ...
    do
        ...
        do
            ...
            # python3 benchmarks/custom_benchmark_latency.py \
            torchrun --standalone --nnodes=1 --nproc-per-node=$tp benchmarks/benchmark_latency.py \
                     ...
                     --tensor-parallel-size $tp --num-iters $ITER --download-dir /data/Mixtral/ $RPD_PROFILE --dtype float16

            # python3 benchmarks/benchmark_latency.py \
            #          --model mistralai/Mixtral-8x7B-v0.1 --batch-size 1 --input-len $input_len --output-len $gen_len \
            #          --tensor-parallel-size $tp --num-iters $ITER --download-dir /data/Mixtral/ # --distributed_executor_backend="torchrun"  
